...
def get_minio_credentials(tenant, strip_http=True, verbose=True, vault_file_template="/vault/secrets/minio-{tenant}-tenant-1"):
    """
    ...
    Returns:
        (dict): Dict with keys:
            ...
            secret_key
    """
    vault = vault_file_template.format(tenant=tenant)
    if verbose:
...
def find_files_matching_pattern(minio_url, bucket, access_key, secret_key,
